...
type clientModelsResponse struct {
	Success     bool                         `json:"success"`
	Data        []clientModelItem            `json:"data"`
	Categories  []clientModelCategorySummary `json:"categories"`
	TotalModels int                          `json:"total_models"`
}

func decodeClientModelsResponse(t *testing.T, recorder *httptest.ResponseRecorder) clientModelsResponse {
	t.Helper()

	require.Equal(t, http.StatusOK, recorder.Code)
	var payload clientModelsResponse
	require.NoError(t, common.Unmarshal(recorder.Body.Bytes(), &payload))
	require.True(t, payload.Success)
	return payload
}

func TestClientListModelsFiltersCategoryAndCountsChannels(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&[]model.Channel{
		{Id: 1, Name: "openai-primary", Status: common.ChannelStatusEnabled},
		{Id: 2, Name: "openai-backup", Status: common.ChannelStatusEnabled},
		{Id: 3, Name: "disabled-video", Status: common.ChannelStatusManuallyDisabled},
	}).Error)
	require.NoError(t, db.Create(&[]model.Ability{
		{Group: "default", Model: "gpt-client-test", ChannelId: 1, Enabled: true},
		{Group: "vip", Model: "gpt-client-test", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "gpt-client-test", ChannelId: 2, Enabled: true},
		{Group: "default", Model: "dall-e-client-test", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "sora-client-test", ChannelId: 1, Enabled: true},
		{Group: "default", Model: "sora-client-test", ChannelId: 3, Enabled: true},
		{Group: "default", Model: "whisper-client-test", ChannelId: 1, Enabled: true},
	}).Error)

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/api/client/models?category=vido", nil)

	ClientListModels(ctx)

	payload := decodeClientModelsResponse(t, recorder)
	require.Equal(t, 1, payload.TotalModels)
	require.Len(t, payload.Data, 1)
	require.Equal(t, "sora-client-test", payload.Data[0].Model)
	require.Equal(t, clientModelCategoryVideo, payload.Data[0].Category)
	require.Equal(t, int64(1), payload.Data[0].ChannelCount)
	require.Len(t, payload.Categories, 1)
	require.Equal(t, clientModelCategoryVideo, payload.Categories[0].Category)
	require.Equal(t, 1, payload.Categories[0].TotalModels)
	require.Equal(t, int64(1), payload.Categories[0].TotalChannels)
}

func TestClientListModelsReturnsOnlyClientCategories(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.Create(&model.Model{ModelName: "disabled-client-all", Status: 2}).Error)
	require.NoError(t, db.Create(&model.Channel{Id: 11, Name: "enabled", Status: common.ChannelStatusEnabled}).Error)
	require.NoError(t, db.Create(&[]model.Ability{
		{Group: "default", Model: "gpt-client-all", ChannelId: 11, Enabled: true},
		{Group: "default", Model: "imagen-client-all", ChannelId: 11, Enabled: true},
		{Group: "default", Model: "veo-client-all", ChannelId: 11, Enabled: true},
		{Group: "default", Model: "bge-client-all", ChannelId: 11, Enabled: true},
		{Group: "default", Model: "disabled-client-all", ChannelId: 11, Enabled: true},
	}).Error)

	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodGet, "/api/client/models", nil)
	...
	for _, item := range payload.Data {
		models[item.Model] = item.Category
	}
	require.Equal(t, map[string]string{
		"gpt-client-all":    clientModelCategoryLLM,
		"imagen-client-all": clientModelCategoryImage,
		"veo-client-all":    clientModelCategoryVideo,
	}, models)
	require.Equal(t, 3, payload.TotalModels)
}
